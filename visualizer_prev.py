...
class TandemRepeatVisualizer:
    ...
    def plot_motif_color_map(symbol_to_motif, motif_counter, symbol_to_color, file_name,
        ...
        if figure_size is not None:
            ...
            w = max_motif_length // 10 + max_motif_length if max_motif_length > 50 else max_motif_length // 5 + 1
            h = len(symbol_to_motif) // 5 + 5 if len(symbol_to_motif) > 50 else len(symbol_to_motif) // 3 + 5
            if h * dpi > 2 ** 16:
    ...
    def trplot(self,
        ...
        if figure_size is None:
            if sample_as_row:
                w = max_repeat_count // 10 + max_repeat_count if max_repeat_count > 50 else max_repeat_count // 5 + 2
                h = len(sample_ids) // 10 + 5 if len(sample_ids) > 50 else 5
            else:
    ...
    def get_symbol_to_color_map(alpha, unique_symbol_count, unique_symbols):
        ...
        import distinctipy
        # TODO support color blind friendly colors
        cmap = distinctipy.get_colors(unique_symbol_count, pastel_factor=0.7, rng=77)
        ...
        # else:
        #     import distinctipy
        #     cmap = distinctipy.get_colors(unique_symbol_count, pastel_factor=0.2, rng=777)
        #     cmap = ListedColormap(cmap)
