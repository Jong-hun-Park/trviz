...
from trviz.utils import INDEX_TO_CHR, PRIVATE_MOTIF_LABEL, get_motif_counter, get_score_matrix
...
class MotifEncoder:
    ...
    def find_private_motif_threshold(decomposed_vntrs, label_count=None):
        ...
        for index, (_motif, count) in enumerate(motif_counter.most_common()):
    ...
    def encode(
        self, decomposed_vntrs: list[list], motif_map_file: str, label_count: int = None, auto: bool = True
    ) -> list[str]:
        """
        ...
        if self.private_motif_threshold > 0:
            normal_motifs, private_motifs = self._divide_motifs_into_normal_and_private(
                motif_counter, self.private_motif_threshold
            )
            if len(normal_motifs) + 1 > len(INDEX_TO_CHR):
                logger.error("Motif counter: %s", motif_counter)
                raise ValueError(
                    f"Too many unique motifs. Can not encode properly: {len(normal_motifs) + len(private_motifs)} unique motifs"
                )
            ...
            if (unique_motif_count := len(motif_counter)) > len(INDEX_TO_CHR):  # single symbol ascii
                raise ValueError(f"Too many unique motifs. Can not encode properly: {unique_motif_count} unique motifs")
